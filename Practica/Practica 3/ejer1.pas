...
begin
    ...
    readln(num_e);
    reset(m);
    seek(m,filesize(m)-1);//Nos posicionamos en el anterior a EOF
    read(m,ult_rec);
    if (ult_rec <> num_e)// nos vamos al principio del archivo
        seek(m,0);
        leer_empleado(m,rec_reg);
        while((rec_reg <> 9999) and (rec_reg.num <> num_e)) do 
            leer_empleado(m,rec_reg);
        if (rec_reg.num <> valor_alto) then begin // si encuentro el dato hago esto.
            seek(m,FilePos(m) - 1);// me paro en el anterior a encontrar el arch
            write(m,rec_reg);
            seek(m,filesize(m)-1)// me paro en la posicion eof - 1
            truncate(m); // trunqueo, o sea muevo el eof a una posicion mas adelante (donde esta el maestro parado)
            writeln('Se elimino el empleado y se coloco el ultimo empleado en su lugar');
            end
            else
                write('no se encontro el empleado ingresado');
    end
    else begin
        seek(m,filesize(m)-1); //si el dato a borrar es el ultimo, directamente lo truncamos moviendono una pos antes q el eof
        truncate(m);
        writeln('se elimno el empelado');
    end
    close(m);
end;
