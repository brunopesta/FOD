...
begin
    ...
    readln(num_e);
    reset(file_emp);
    seek(file_emp,FileSize(file_emp)-1); // Me posiciono en el anterior a eof
    read(file_emp,last_rec);//Lee el ultimo registro del archiv0 empleado
    if (last_rec.num <> num_e) then begin //Si el ultimo es diferente al dato a eliminar
        seek(file_emp,0); //Nos vamos al inicio del archivo
        leer_empleado(file_emp,rec_del);
        while ((rec_del.num <> 9999) and (rec_del.num <> num_e)) do
            leer_empleado(file_emp,rec_del);
        if (rec_del.num <> valor_alto)then begin // si encuentro el dato hago lo siguiente
            seek(file_emp,FilePos(file_emp)-1)//Me paro en el anterior al encontrado
            write(file_emp,last_rec);// obviamente llegue aca si el ultimo no era el dato a borrar, traigo el ultimo aca
            seek(file_emp,FileSize(file_emp)-1);// ,e me paro en la posicion de eof -1
            truncate(file_emp)// aca trunqueo, basicamente muevo el eof a una posicion menos
            writeln('Se elimino el empleado y se coloco el ultmo empleado en su lugar');
            end
            else
                write('No se encotro el empleado ingresado');
    end
    else begin
        seek(file_emp, FileSize(file_emp)-1)//Si el dato a borrar era el ultimo, directamente se hace el trunqueo
        truncate(file_emp);
